...
$task.fetch(myRequestV4).then(responseV4 => {
  ipv4Info = responseV4 ? json1info(responseV4.body, paras) : "";
  // 请求 IPv6 地理位置信息
  $task.fetch(myRequestV6).then(responseV6 => {
    ipv6Info = responseV6 ? json2info(responseV6.body, paras) : "";
    // 合并 IPv4 和 IPv6 信息并显示
    message = `<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: bold;">IP查询结果</p>` + ipv4Info + `<br>` + ipv6Info;
...
function json2info(cnt, paras) {
  ...
  for (i = 0; i < paras.length; i++) {
    ...
    res = cnt[paras[i]] ? res + "</br><b>" + "<font  color=#FFFFFF>" + param[i] + "</font> : " + "</b>" + "<font  color=#FFFFFF>" + cnt[paras[i]] + "</font></br>" : res;
  }
...
function json1info(cnt, paras) {
  ...
  i = 0;
  cnt[paras[i]] = paras[i] == "country_code" ? cnt[paras[i]] + " ⟦" + flags.get(cnt[paras[i]].toUpperCase()) + "⟧" : cnt[paras[i]];
  res = cnt[paras[i]] ? res + "</br><b>" + "<font  color=#FFFFFF>" + paran[i] + "</font> : " + "</b>" + "<font  color=#FFFFFF>" + cnt[paras[i]] : res;
  res = `<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">` + res + `</p>`;
  return res;
}
